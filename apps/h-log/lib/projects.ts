...
export const projects = [
  ...
  },
  {
    ...
    icon: Clapperboard,
    impact: [
      "캐시 적용으로 피크 시간대 티켓·차량 조회 지연을 줄임",
      "결제 후 알림을 비동기로 분리해 결제 응답 경로를 단축",
      "Chunk 처리와 동시성 제어로 대량 데이터 처리 안정성을 높임",
    ],
  ...
  },
  {
    ...
    icon: BarChart3,
    impact: [
      "레거시 기능을 나눠 이관해 변경 리드타임과 운영 전환 위험을 줄임",
      "조회 구조를 정리해 불필요한 쿼리와 API 응답 지연을 줄임",
      "복합 인덱스로 자주 사용하는 검색 조건의 실행 속도를 개선",
    ],
  ...
  },
  {
    ...
    icon: MessageSquare,
    impact: [
      "수동 발송을 API 기반 자동 처리 흐름으로 전환",
      "조회 쿼리와 인덱스를 정리해 CRM 화면 응답 지연을 줄임",
  ...
  },
  {
    ...
    icon: Database,
    impact: [
      "테이블과 Stored Procedure를 검증 가능한 단위로 전환",
  ...
  },
  {
    ...
    icon: ServerCog,
    impact: [
      "반복 장애 패턴을 분류하고 재발 방지 작업을 적용해 운영 안정성을 높임",
      "쿼리와 인덱스를 정리해 주요 조회 API의 응답 지연을 줄임",
      "Chunk 기반 재시작으로 배치 실패 시 복구 부담을 낮춤",
    ],

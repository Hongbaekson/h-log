...
export const projects = [
  {
    ...
    company: "오프너드",
    context: "Go 기반 Discord와 GitHub Issues 운영 자동화 설계",
    detail: {
      architecture: [
        "GitHub Issues webhook을 회사 HTTPS endpoint에서 raw body 기준으로 수신하고 signature와 delivery id로 신뢰 경계를 검증",
        "Go Backend Receiver가 이벤트를 저장한 뒤 PGMQ Queue로 Discord 알림, GitHub 조회, LLM 요약 작업을 비동기 분리",
        "LLM Worker는 최소 정보만 받아 요약·intent 분석을 수행하고 변경 작업은 preview와 사용자 승인 후 Backend가 GitHub App API로 실행",
      ],
      decisions: [
        "수동 배포는 10분 이상 소요되고, 휴먼 에러와 배포 아티팩트 무결성 검증 부재로 운영 리스크가 있었습니다.",
        "Discord 전송 실패나 rate limit이 webhook 수신 실패로 번지면 GitHub 재전송과 PGMQ 기반 중복 알림 관리가 어려워질 수 있었습니다.",
        "LLM 요약과 담당자 지정·라벨 변경 같은 write 작업을 바로 실행하면 권한·승인·감사 경계가 흐려질 수 있었습니다.",
      ],
      role: [
    ...
    impact: [
      ...
      "빌드·테스트·패키징·체크섬 생성·GPG 서명 검증·S3 업로드를 자동화해 배포 시간을 평균 3분으로 단축하고, 압축 파일·서명 파일·체크섬 기반의 산출물 위변조 검증 흐름을 확보",
      "Discord 429, LLM timeout, 알림 실패가 webhook 수신에 영향을 주지 않도록 PGMQ retry, archive, audit 흐름을 분리",

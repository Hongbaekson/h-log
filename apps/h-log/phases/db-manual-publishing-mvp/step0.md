...
- production target은 OCI에서 운영하는 PostgreSQL + pgvector다. 다만 이 step에서는 실제 OCI DB 접속 없이 local/test double 또는 코드 contract로 검증한다.
...
- 실제 운영 DB, OCI 서버, 외부 서비스에 연결하지 말 것. 이유: 이 step은 model contract 확정 단계다.
- 방문자 챗봇을 추가하지 말 것.

...
**트레이드오프**: 아직 실제 PostgreSQL/Hermes one-shot runner와 required job adapter가 없으므로 이 결정만으로 production 자동 발행은 동작하지 않는다. Server runner는 같은 day key의 durable 중복 여부를 DB에서 먼저 확인하고, worker adapter는 publish 전 내부 검증과 publish 후 public smoke의 순서를 별도로 검증해야 한다.

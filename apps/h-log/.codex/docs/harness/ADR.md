...
**결정**: 검색, 관련 글 추천, 임베딩은 내부 발행/탐색 품질을 위해 사용하되 방문자 RAG 챗봇 UI는 만들지 않는다. 실제 embedding provider가 없는 현재 public search route는 keyword-only fallback을 사용하고, fake provider 호출이나 usage 기록을 만들지 않는다.

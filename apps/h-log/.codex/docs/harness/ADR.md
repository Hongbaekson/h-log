...
**이유**: 현재 PostgreSQL schema/repository/public read path와 manual `--once` worker는 구현됐지만, fake-provider end-to-end dry-run과 실제 provider/scheduler 활성화는 남아 있다. 계약이 충분하다는 이유로 성과 학습을 먼저 진행하면 검증된 운영 데이터와 실패 신호가 없는 상태에서 최적화 계약만 늘어난다.

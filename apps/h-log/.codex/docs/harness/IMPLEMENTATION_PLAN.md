...
수정된 `plans/automated-blog-publishing-plan.md` 기준으로 블로그 본선은 DB-first다. 기존 file-based loader, caller가 없던 post-publish verification facade, unwired diagram 계획·저장·실패·감사 helper, caller-free admin workflow, stale table/field registry와 unwired generation-run factory는 `runtime-contract-pruning / Steps 0-4`에서 전용 테스트와 함께 제거했다. published-current crawler manifest, live required adapter/worker, diagram render predicate, repository-backed retract/audit, domain validation과 persistence는 유지한다.
...
runtime-contract-pruning: pending, Steps 0-4 legacy loader, caller-free verification facade, unwired diagram helper, caller-free admin workflow, and test-only model mirror removal completed; Steps 5-9 remain
...
- Steps 0-10은 완료했다.
- 다음 local 실행 대상은 `runtime-contract-pruning / Step 5: remove-test-only-public-data-fixture`다.
...
10. `runtime-contract-pruning / Steps 0-9`: Steps 0-4에서 live caller가 없는 legacy file loader, post-publish verification facade, diagram 계획·저장·실패·감사 helper, admin preview/draft/publish/correction/unpublish/generic operational action과 test-only model mirror를 제거했다. 이후에도 각 step 시작 시 live caller를 다시 확인한 뒤 public fixture와 repository write API, 중복 slug proxy를 삭제한다. 이어 confirmed-unused worker mode/egress와 Compose/systemd의 동일 runtime override만 제거한다. DB-backed public/crawler/retract/audit/rendering, scheduler egress, OAuth preflight 경계는 유지한다.

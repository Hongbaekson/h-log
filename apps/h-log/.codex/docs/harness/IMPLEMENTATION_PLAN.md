...
- 상태: completed
- 결과: `lib/blog-article-generation.ts`와 테스트로 `experiment` mode가 concrete experiment evidence path 없이 통과하지 못하도록 고정했다. 당시 caller-free `createArticleGenerationRunRecord`가 `post_generation_runs` 형태를 만들었지만 live persistence가 없어 `runtime-contract-pruning / Step 4`에서 제거했다. Persona는 style/version 경계이며 factual claim verification을 대체하지 않는다. 실제 LLM 호출, DB 저장, 공개 발행 side effect는 추가하지 않았다.
...
- Steps 0-10은 완료했다.
- 다음 local 실행 대상은 `runtime-contract-pruning / Step 6: remove-test-only-repository-write-apis`다.
...
10. `runtime-contract-pruning / Steps 0-9`: Steps 0-5에서 live caller가 없는 legacy file loader, post-publish verification facade, diagram 계획·저장·실패·감사 helper, admin preview/draft/publish/correction/unpublish/generic operational action, test-only model mirror와 public fixture를 제거했다. 이후에도 각 step 시작 시 live caller를 다시 확인한 뒤 repository write API와 중복 slug proxy를 삭제한다. 이어 confirmed-unused worker mode/egress와 Compose/systemd의 동일 runtime override만 제거한다. DB-backed public/crawler/retract/audit/rendering, scheduler egress, OAuth preflight 경계는 유지한다.
...
### generation-integrity-hardening / Step 1: hermes-no-tool-enforcement

- 상태: completed
- 결과: Hermes writer는 `--safe-mode --toolsets context_engine`으로 실행해 tool definition 0개를 강제한다. `HLOG_HERMES_MODEL`과 factory `model` override를 제거하고 provider 내부의 `openai-codex`/`gpt-5.6-sol` 단일값을 invocation과 usage 검증에 사용한다. Child process에서는 `HERMES_KANBAN_TASK` 상속을 제거해 kanban toolset 자동 추가도 차단한다.
- RED: 기존 invocation에 safe mode가 없고 environment/factory override가 각각 `env-override`, `caller-override`를 전달하는 focused 2 failure와 kanban 환경 변수 제거 helper 부재를 확인했다.
...
### search-runtime-alignment / Step 0: keyword-search-without-fake-embedding

- 상태: completed
- 결과: public `/api/search` route에서 `keyword-only`/`h-log-local` fake embedding adapter와 그 adapter에만 필요했던 route-level budget/ledger wiring을 제거했다. 실제 provider가 없는 현재 route는 existing keyword fallback만 사용하며 embedding `usage_events` row를 만들지 않는다. Optional real adapter의 budget/ledger contract와 related-post vector 계산은 유지했다.
- RED: route source에 `localKeywordOnlyEmbeddingAdapter`가 남아 있어 fake embedding accounting 회귀 테스트 1건이 기대한 이유로 실패했다.
- 검증: focused GREEN 14/14, 전체 `npm run test` 179개 중 167 pass/12 DB environment skip, `npm run typecheck`, `npm run lint`, `npm run build` 통과.
- 운영 경계: schema, dependency, environment, external provider, OCI, timer를 변경하지 않았다.
- 다음 local 실행 대상: `search-runtime-alignment / Step 1: guard-before-public-store-load`.

### search-runtime-alignment / Step 1: guard-before-public-store-load

- 상태: completed
- 결과: public `/api/search` route가 PostgreSQL store를 미리 읽지 않고 existing handler에 loader를 전달한다. Handler는 short, abnormal, duplicate, rate-limited request를 loader 호출 0회로 차단하고, allowed request에서 한 번 읽는다. Cache hit는 current published store를 다시 읽어 retracted post를 제거한다.
- RED: lazy loader를 전달한 focused test가 handler의 eager `input.store.posts` 접근에서 실패했다.
- 검증: focused GREEN 15/15, 전체 `npm run test` 180개 중 168 pass/12 DB environment skip, `npm run typecheck`, `npm run lint`, `npm run build`, JSON parse, `git diff --check` 통과.
- 운영 경계: SQL, cache policy, request history 상한, client IP trust, schema, dependency, environment, OCI, timer를 변경하지 않았다.
- 다음 local 실행 대상: `search-runtime-alignment / Step 2: submitted-query-ui-consistency`.

### search-runtime-alignment / Step 2: submitted-query-ui-consistency

- 상태: completed
- 결과: 검색 input draft는 계속 편집할 수 있게 두고 결과 상태에는 제출 당시 query snapshot을 표시했다. 각 요청에 증가하는 request id를 부여해 이전 요청의 늦은 success/error 응답이 최신 제출 결과를 덮어쓰지 않게 했다. 새 state framework, request queue, dependency는 추가하지 않았다.
...
### runtime-contract-pruning / Step 2: shrink-diagram-asset-contract

- 상태: completed
- 결과: repository-wide caller를 다시 확인해 `lib/blog-diagram-assets.ts`에서 public renderer가 사용하는 `isRenderableDiagramAsset`과 입력 type만 유지했다. 전용 테스트 외 caller가 없던 diagram generation planning, storage, failure, audit helper와 type을 전용 테스트와 함께 제거했다.
- 검증: 삭제 전 characterization 16/16, 삭제 후 public rendering focused 7/7, 전체 `npm run test` 165개 중 153 pass/12 DB environment skip, `npm run typecheck`, `npm run lint`, `npm run build`, JSON parse, removed-symbol scan, `git diff --check`가 통과했다.
- 운영 경계: 현재 post version, `ready` status, SHA-256 asset hash, verified timestamp, figure insertion 위치를 유지했다. `post_assets` row, 기존 SVG, schema, dependency, environment, OCI, timer는 변경하지 않았고 새 generation/storage pipeline도 추가하지 않았다.
- 다음 local 실행 대상: `runtime-contract-pruning / Step 3: shrink-admin-contract`.

### runtime-contract-pruning / Step 3: shrink-admin-contract

- 상태: completed
- 결과: repository-wide caller를 다시 확인해 `lib/blog-admin.ts`에서 PostgreSQL repository가 사용하는 `retractAdminPost`, `AdminPostVisibilityInput`, 최소 store type만 유지했다. 전용 테스트 외 caller가 없던 preview, draft, publish, correction, unpublish, generic operational action 함수/type/test는 제거했다.
- 검증: 삭제 전 characterization 8/8, 삭제 후 focused 2/2, PostgreSQL integration 13/13, 전체 `npm run test` 159개 중 147 pass/12 DB environment skip, `npm run typecheck`, `npm run lint`, `npm run build`가 통과했다.
- 운영 경계: post status와 `admin_actions`의 transaction/rollback, audit text 검증, public/crawler/search/related 제거를 유지했다. `/admin`, auth, Discord command, 새 API, schema, dependency, environment, OCI, timer는 변경하지 않았다.
- 다음 local 실행 대상: `runtime-contract-pruning / Step 4: remove-test-only-model-mirrors`.

### runtime-contract-pruning / Step 4: remove-test-only-model-mirrors

- 상태: completed
- 결과: repository-wide caller와 migrations `001`-`003`, repository query를 다시 확인했다. 실제 migration과 어긋난 `BLOG_CONTENT_MODEL_TABLES` table/field registry 및 field-list assertions, live caller와 persistence가 없던 `createArticleGenerationRunRecord` factory/input type/dedicated test를 제거했다.
- 검증: 삭제 전 characterization 35/35, 삭제 후 article/content-model/daily focused 33/33, 전체 `npm run test` 157개 중 145 pass/12 DB environment skip, `npm run typecheck`, `npm run lint`, `npm run build`, phase JSON parse, removed-symbol scan, `git diff --check`가 통과했다.
- 운영 경계: migrations와 repository query를 DB schema source of truth로 유지했다. live domain record type, article validation, content hash, post 상태 전이, aggregate persistence, usage ledger는 변경하지 않았고 새 migration/table/registry/dependency/environment/OCI/timer도 추가하지 않았다.
- 다음 local 실행 대상: `runtime-contract-pruning / Step 5: remove-test-only-public-data-fixture`.

### runtime-contract-pruning / Step 5: remove-test-only-public-data-fixture

- 상태: completed
- 결과: repository-wide caller를 다시 확인해 `lib/blog-public-data.ts`의 유일한 import가 inline-code 단위 테스트임을 확인했다. 해당 테스트는 같은 파일의 기존 `createStore`와 `createVersion` builder를 사용하도록 바꾸고 runtime caller가 없던 200줄 fixture를 삭제했다.
- 검증: 변경 전후 focused 7/7, 전체 `npm run test` 157개 중 145 pass/12 DB environment skip, `npm run typecheck`, `npm run lint`, `npm run build`, phase JSON parse, removed-import scan, `git diff --check`가 통과했다.
- 운영 경계: PostgreSQL-backed public source, published-current selector, Markdown renderer와 inline-code 출력을 유지했다. 정적 fallback, shared fixture, DB/user content/schema/dependency/environment/OCI/timer 변경은 추가하지 않았다.
- 다음 local 실행 대상: `runtime-contract-pruning / Step 6: remove-test-only-repository-write-apis`.

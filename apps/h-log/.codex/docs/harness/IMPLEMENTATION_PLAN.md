...
- 운영 경계: 2026-07-22 OCI read-only preflight에서 기준 경로는 root-level older source이고 host에는 Docker만 있으며 Node/npm/Hermes, production env, timer가 없음을 확인했다. Scheduler package와 timer는 저장소에만 추가했고 서버에 build/auth/install/enable하지 않았다. Container-local OAuth, production input mount, migration 전 backup/restore rehearsal, canary 1건, live rollback smoke가 끝날 때까지 Step 4와 phase는 완료 처리하지 않는다.
- 다음 실행 대상: 최신 artifact를 OCI 기준 경로에 반영하고 `hlog-auto-publish` image를 build한 뒤 `hermes_data` volume에 OpenAI Codex OAuth를 등록한다. Production input/env mount와 backup/restore rehearsal을 확인한 후 timer를 켜기 전에 수동 canary 1건과 rollback smoke를 실행한다.

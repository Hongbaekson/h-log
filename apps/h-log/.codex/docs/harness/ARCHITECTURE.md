...
- 프로필 사진과 GitHub는 공개할 수 있지만 이메일과 Contact form은 공개하지 않는다. PDF는 조직 식별자 일반화와 `이력서` 명칭 통일 후 공개한다.

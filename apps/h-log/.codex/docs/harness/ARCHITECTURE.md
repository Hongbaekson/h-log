...
- `lib/blog-usage-ledger.ts`: `usage_events` 멱등 기록과 UTC 일/월 estimated cost 합계를 제공한다. 비용성 adapter는 호출 전에 이 원장을 요구하고, 일/월 한도 도달 시 `budget_exceeded`로 차단한다. 현재 `/api/search` route는 embedding adapter를 주입하지 않고 native keyword fallback을 사용하므로 embedding usage를 기록하지 않는다. Future real adapter를 명시적으로 주입하는 handler contract는 기존 budget/ledger gate를 유지한다.

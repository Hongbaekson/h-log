...
위 자동화 항목은 현재 순수 contract/test baseline이다. PostgreSQL `pg` driver, `001_blog_core` SQL migration, migration runner와 local Compose 통합 테스트는 구현됐다. Public blog는 아직 `lib/blog-public-data.ts`의 정적 store를 읽으며, Compose worker는 자동 job이 없는 placeholder command다. 따라서 실제 DB repository, DB-backed public read path, persistent worker, 외부 provider, scheduler가 동작하는 production 자동 발행 runtime은 아직 구현되지 않았다.
...
첫 SQL migration은 local Compose PostgreSQL에서 검증됐다. Public read 흐름은 아직 정적 `blogContentStore`와 순수 contract를 사용하며, 다음 `blog-runtime-integration / Step 1`부터 아래 local runtime vertical slice를 이어간다.

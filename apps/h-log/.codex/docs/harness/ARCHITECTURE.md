...
`auto-publish-ops-hardening` Step 4의 local rollback readiness, Hermes Codex OAuth article provider local smoke, private persistence handoff, PostgreSQL/Hermes one-shot runner, required job adapter, bounded Compose/systemd scheduler packaging까지 완료했다. OCI artifact와 container-local Hermes OAuth, pre-migration logical backup 및 격리 restore/migration rehearsal도 검증했다. Server-local credential/env/input, live migration, 공개 canary, timer 활성화와 live rollback smoke가 남아 있으므로 phase는 pending이다.

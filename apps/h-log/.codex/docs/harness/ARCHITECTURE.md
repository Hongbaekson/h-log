...
`auto-publish-ops-hardening` Step 4의 local rollback readiness, Hermes Codex OAuth article provider local smoke, private persistence handoff까지 완료했다. Provider/model은 `openai-codex`/`gpt-5.6-sol`로 결정됐지만 server-local Hermes OAuth, Hermes/DB one-shot runner, required job adapter packaging, 09:00 KST scheduler, OCI migration 배포, 공개 canary와 live rollback smoke가 남아 있으므로 phase는 pending이다.
...
Hermes article provider는 이 `generateArticle` adapter를 구현한다. 실행마다 provider/model을 명시하고 AGENTS/rules/memory 주입을 끈 뒤 verified input만 prompt에 넣는다. 반환 JSON은 기존 `validateArticleWriterOutput`과 privacy scanner를 그대로 통과해야 하며, 실제 model은 `post_generation_runs`와 `usage_events`에 기록한다. Local Windows smoke에서 `openai-codex`, `gpt-5.6-sol`, `included`, estimated cost 0을 확인했지만 현재 Compose worker image에는 Hermes가 포함되지 않았고 scheduler도 없으므로 production data flow에는 아직 연결하지 않는다.

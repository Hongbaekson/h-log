...
`auto-publish-ops-hardening` Step 4의 local rollback readiness와 Hermes Codex OAuth article provider local smoke까지 완료했다. Provider/model은 `openai-codex`/`gpt-5.6-sol`로 결정됐지만 server-local Hermes 설치/OAuth, worker packaging, 09:00 KST scheduler, OCI migration 배포, 공개 canary와 live rollback smoke가 남아 있으므로 phase는 pending이다.

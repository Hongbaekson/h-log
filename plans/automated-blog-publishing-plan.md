...
- `runtime-contract-pruning` Steps 0-6은 전용 테스트 외 live consumer가 없는 파일 기반 blog loader, post-publish verification facade, diagram 계획·저장·실패·감사 helper, admin preview/draft/publish/correction/unpublish/generic operational action, stale table/field registry, generation-run factory, test-only public-data fixture와 repository write API를 제거했다. published-current crawler manifest, live required adapter/worker, diagram render predicate, repository-backed retract/audit, DB-backed public source, live domain validation과 persistence는 유지했다. Publish-job setup은 기존 `savePost` aggregate 경로, rollback verification setup은 test-local SQL로 옮겼으며 internal insertion과 기존 검증은 유지했다. Steps 7-9는 매 step live caller를 재확인한 뒤 중복 slug proxy, unused worker capability와 중복 container runtime default만 삭제한다.

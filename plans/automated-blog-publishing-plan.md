...
- `runtime-contract-pruning` Steps 0-2는 전용 테스트 외 live consumer가 없는 파일 기반 blog loader, post-publish verification facade, diagram 계획·저장·실패·감사 helper를 제거했다. published-current crawler manifest, live required adapter/worker, diagram render predicate는 유지했다. Steps 3-9는 매 step live caller를 재확인한 뒤 unwired/test-only contract, 중복 slug proxy, unused worker capability와 중복 container runtime default만 삭제한다.

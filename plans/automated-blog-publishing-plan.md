...
- DB content model, published-only route, 검색/SEO, 연구/생성, daily pipeline, `post_assets` 기반 diagram insertion gate는 contract/test baseline까지 완료됐다. live caller가 없는 diagram 생성·저장 helper는 후속 pruning에서 제거했다.
...
- `runtime-contract-pruning` Steps 0-7은 전용 테스트 외 live consumer가 없는 파일 기반 blog loader, post-publish verification facade, diagram 계획·저장·실패·감사 helper, admin preview/draft/publish/correction/unpublish/generic operational action, stale table/field registry, generation-run factory, test-only public-data fixture/repository write API와 중복 slug proxy를 제거했다. published-current crawler manifest, live required adapter/worker, diagram render predicate, repository-backed retract/audit, DB-backed public source, page-owned detail 404, Markdown rewrite, live domain validation과 persistence는 유지했다. `/blog` loading은 index-only route group이 소유하며, Publish-job setup은 기존 `savePost` aggregate 경로, rollback verification setup은 test-local SQL을 사용한다. Step 8은 미사용 worker 모드 설정과 manual worker egress를 제거했다. Worker는 internal Nginx/PostgreSQL만 사용하고 Hermes outbound는 `hlog-auto-publish`에 유지한다. Step 9는 live caller를 재확인한 뒤 중복 container runtime default만 삭제한다.
...
7. generation integrity와 keyword search fake accounting 제거 완료, 남은 search runtime/runtime/public surface pruning - pending local follow-up
...
2026-08-28 live audit에서 세 local follow-up phase를 추가했다. `generation-integrity-hardening`은 claim verifier 연결, no-tool writer/단일 model 경로, redacted 실패 사유의 one-shot handoff까지 완료했다. `search-runtime-alignment`는 현재 keyword-only runtime의 fake accounting과 blocked query eager DB read를 제거하고 submitted query와 표시 결과를 일치시켰으며, 이전 요청의 늦은 응답이 최신 결과를 덮어쓰지 않게 했다. 다음 local phase인 `runtime-contract-pruning`은 Steps 0-7에서 legacy/unwired contract, caller-free admin workflow, test-only model mirror, public-data fixture/repository write API와 중복 slug proxy를 제거했으며, Step 8에서 미사용 worker 모드/egress도 제거했다. 다음 Step 9는 live caller를 재확인하고 중복 container runtime default만 삭제한다. 2026-08-31 audit에서 native redirect와 public blog 표시 규칙을 다루는 `public-surface-refactor-pruning`을 추가했다. 이 follow-up도 실제 provider, domain, DNS/TLS, OCI mutation, timer activation을 포함하지 않으며, 완료 후에도 HTTPS origin과 privacy 목록을 받는 `auto-publish-ops-hardening / Step 4` 승인 gate를 유지한다.
...
현재 daily runtime은 writer schema와 privacy/article quality gate 통과 후 normalized claim을 같은 run의 verified `postSources`로 `verifyArticleClaims`에 전달한다. Unknown/discovery-only/contradicted/unsupported factual claim은 post/version 생성과 private persistence 전에 `generation_failed`로 종료한다.
...
```json
{
  ...
  "description": "...",
  "tags": ["..."],
  "article_mode": "experiment | applied_analysis | document_analysis | project_record | ops_incident",
  "content_markdown": "...",
  "claims": [
    {
      "text": "...",
...
- apply_categories
- apply_targets
- mode: experiment | applied_analysis | document_analysis | project_record | ops_incident
- hypothesis
- commands_or_checks
- evidence_path
- summary
...
daily-blog-cron
  ...
  -> validateArticle()
  -> verifyArticleClaims(postSources)
...
- 검색 API의 optional real embedding adapter 비용/봇 방어 contract - 완료. 현재 keyword-only route는 embedding 호출이나 usage row를 만들지 않음
